...
def evaluate_model(
    ...
    """
    # If return_predictions=True: return Tuple scores, predictions.
    ...
    for inputs, labels in progressbar(
        ...
        for i in range(0, len(predicted)):
            ...
            if return_predictions:
                predictions.append((src_word, predicted[i] == idx, key, idx_to_class[predicted[i].item()]))
...
def create_tensors(
    tokens, activations, task_specific_tag, mappings=None, model_type="classification", return_torch=False, x_dtype='float32'
):
    assert (
        model_type == "classification" or model_type == "regression"
    ), "Invalid model type"
    ...
    if mappings is not None:
        ...
        if model_type == "classification":
            ...
            NN_idx = max(label2idx.values()) + 1
            label2idx[task_specific_tag] = NN_idx
    ...
    if model_type == "classification":
        if return_torch:
            ...
            del X, y
            return X_torch, y_torch, (label2idx, idx2label, src2idx, idx2src)
