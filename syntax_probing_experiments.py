...
sys.path.append(os.path.abspath('bin/aux_classifier/'))
...
def findMinimalNeurons(tensors, train_tokens, dev_tokens, L1=0.0001, L2=0.0001, MODEL='aux_model.pt', idx2label=dict(), JSON='mappings.json', results=dict()):
    ...
    ordering, cutoffs = utils.get_neuron_ordering(
        root_model, label2idx, search_stride=1000)
    ...
    for percentage in [0.01, 0.02, 0.03, 0.05, 0.1, 0.2, 0.3, 0.5]:
        ...
        X_filtered = utils.filter_activations_keep_neurons(
            random_neurons, tensors['X'])
        X_dev_filtered = utils.filter_activations_keep_neurons(
            random_neurons, tensors['X_dev'])
        model_temp = utils.train_logreg_model(
            X_filtered, tensors['y'], lambda_l1=L1, lambda_l2=L2, num_epochs=10, batch_size=config['batch_size'])
        results[str(percentage)+'_random_train'] = utils.evaluate_model(
            model_temp, X_filtered, tensors['y'], idx2label, source_tokens=train_tokens['source'])
        results[str(percentage)+'_random_dev'] = utils.evaluate_model(
            model_temp, X_dev_filtered, tensors['y_dev'], idx2label, source_tokens=dev_tokens['source'])
        ...
        X_filtered = utils.filter_activations_keep_neurons(
            bottom_neurons, tensors['X'])
        X_dev_filtered = utils.filter_activations_keep_neurons(
            bottom_neurons, tensors['X_dev'])
        model_temp = utils.train_logreg_model(
            X_filtered, tensors['y'], lambda_l1=L1, lambda_l2=L2, num_epochs=10, batch_size=config['batch_size'])
        results[str(percentage)+'_bot_train'] = utils.evaluate_model(
            model_temp, X_filtered, tensors['y'], idx2label, source_tokens=train_tokens['source'])
        results[str(percentage)+'_bot_dev'] = utils.evaluate_model(
            model_temp, X_dev_filtered, tensors['y_dev'], idx2label, source_tokens=dev_tokens['source'])
...
def leftRightBaseline(config, train_tokens, dev_tokens, tensors, idx2label, results, duplicate=False):
    ...
    X_left = utils.filter_activations_keep_neurons(leftneurons, tensors['X'])
    X_dev_left = utils.filter_activations_keep_neurons(
        leftneurons, tensors['X_dev'])
    X_right = utils.filter_activations_keep_neurons(rightneurons, tensors['X'])
    X_dev_right = utils.filter_activations_keep_neurons(
        rightneurons, tensors['X_dev'])
    ...
    model_left = utils.train_logreg_model(
        X_left, tensors['y'], lambda_l1=config['L1'], lambda_l2=config['L2'], num_epochs=10, batch_size=config['batch_size'])
    results[exp_name_left+'train'] = utils.evaluate_model(model_left, X_left, tensors['y'], idx2label, source_tokens=train_tokens['source'])
    results[exp_name_left+'dev'] = utils.evaluate_model(model_left, X_dev_left, tensors['y_dev'], idx2label, source_tokens=dev_tokens['source'])
    print('train and evaluate models on right token only')

    model_right = utils.train_logreg_model(
        X_right, tensors['y'], lambda_l1=config['L1'], lambda_l2=config['L2'], num_epochs=10, batch_size=config['batch_size'])
    results[exp_name_right+'train'] = utils.evaluate_model(model_right, X_right, tensors['y'], idx2label, source_tokens=train_tokens['source'])
    results[exp_name_right+'dev'] = utils.evaluate_model(model_right, X_dev_right, tensors['y_dev'], idx2label, source_tokens=dev_tokens['source'])
...
def layerWiseProbing(config, train_activations, dev_activations, train_tokens, dev_tokens, results=dict()):

    layerAccuracy_train = [0] * config['num_layers']
    layerAccuracy_dev = [0] * config['num_layers']
    layerIndex = list(range(1, config['num_layers']+1))
    ...
    for layer in range(1, config['num_layers']+1):
        ...
        f_train_activations, f_dev_activations = utils.filter_activations_by_layers(
            train_activations, dev_activations, filter_layers, config['num_neurons_per_layer'], config['num_layers'], config['is_brnn'])

        print("Creating tensors tensors...")
        X_filtered, y, mappings = utils.create_tensors(
            train_tokens, f_train_activations, config['task_specific_tag'])
        X_dev_filtered, tensors['y_dev'], mappings = utils.create_tensors(
            dev_tokens, f_dev_activations, config['task_specific_tag'], mappings)
        label2idx, idx2label, src2idx, idx2src = mappings

        print("Building model...")

        model_filtered = utils.train_logreg_model(
            X_filtered, y, lambda_l1=config['L1'], lambda_l2=config['L2'], num_epochs=10, batch_size=config['batch_size'])
        overall_train_accuracies = utils.evaluate_model(
            model_filtered, X_filtered, y, idx2label, source_tokens=train_tokens['source'])
        overall_dev_accuracies, predictions = utils.evaluate_model(
        ...
        del f_train_activations, f_dev_activations
    ...
    while (len(top_neurons_local) < len(top_neurons_global)):
        top_neurons_local, classWise = utils.get_top_neurons(
            model, percentage*fac, label2idx)
        fac = fac + 0.1
...
if __name__ == '__main__':
    ...
    tensors['X'], tensors['y'], tensors['mappings'] = utils.create_tensors(
        train_tokens, train_activations, config['task_specific_tag'], return_torch=True, x_dtype='float16')
    ...
    tensors['X_dev'], tensors['y_dev'], tensors['mappings'] = utils.create_tensors(
        dev_tokens, dev_activations, config['task_specific_tag'], mappings = tensors['mappings'], return_torch=True, x_dtype='float16')
    ...
    print("Building model...")
    model = utils.train_logreg_model(tensors['X'], tensors['y'], lambda_l1=0,
                                     lambda_l2=0, num_epochs=config['num_epochs'], batch_size=config['batch_size'])
    overall_train_accs, general_train_preds = utils.evaluate_model(model, tensors['X'], tensors['y'], idx2label, return_predictions=True)
    results['general_acc_train'] = overall_train_accs
    overall_dev_accs, general_dev_preds = utils.evaluate_model(model, tensors['X_dev'], tensors['y_dev'], idx2label, return_predictions=True, source_tokens=dev_tokens['source'])
    ...
    if not(config['no_detailed_analysis']):
        ...
        results = layerWiseProbing(config, train_activations, dev_activations, train_tokens, dev_tokens, results=results)
